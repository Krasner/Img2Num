...
export function useWasmWorker() {
  ...
  const imageToSVG = async (         { pixels, width, height, minArea }) => {
    ...
    const outSvgData = (await call('imageToSVG', { pixels, width, height, minArea, outSvg, outSvgSizeLimit }, ['pixels', 'outSvg'])).output.outSvg;

    // Find the null terminator
    const zeroIndex = outSvgData.indexOf(0);
    const svg = new TextDecoder().decode(outSvgData.subarray(0, zeroIndex));
    return svg;
  };
